...
#ifdef DEBUG
  setup_debug_instr(reg.A,0);
  setup_debug_instr(reg.F,1);
  setup_debug_instr(reg.B,2);
  setup_debug_instr(reg.C,3);
  setup_debug_instr(reg.D,4);
  setup_debug_instr(reg.E,5);
  setup_debug_instr(reg.H,6);
  setup_debug_instr(reg.L,7);
  setup_debug_instr(reg.sp,reg.pc);
#endif
  //int a = 0x01;
  //int b = 0x02;
  //int c2 = 0x04;
...
#ifdef DEBUG
    std::cout << std::hex << "Register values: PC: " <<int(reg.pc) << ", SP: " << int(reg.sp) << ", A: " << int(reg.A) << ", F: " << BIN(reg.F) << ", AF: " << int(reg.AF) << ", B: " << int(reg.B) << ", C: " << int(reg.C) << ", BC: " << int(reg.BC) << ", D: " << int(reg.D) << ", E: " << ", DE: " << int(reg.DE) << ", H:" << int(reg.H) << ", L:" << int(reg.L) << ", HL: " << int(reg.HL) << std::endl;
