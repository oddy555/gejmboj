...
/*  reg.A = 0;
  ...
  reg.AF = Register16(Register8(0,"F"),Register8(0,"A");
  reg.A = &reg.AF.getHigh();
  reg.F = &reg.AF.getLow();
  reg.BC = Register16(Register8(0,"C"),Register8(0,"B");
  reg.B = &reg.BC.getHigh();
  reg.C = &reg.BC.getLow();
  reg.DE = Register16(Register8(0,"E"),Register8(0,"E");
  reg.D = &reg.DE.getHigh();
  reg.E = &reg.DE.getLow();
  reg.HL = Register16(Register8(0,"L"),Register8(0,"H");  
  reg.H = &reg.HL.getHigh();
  reg.L = &reg.HL.getLow();
...
/*#ifdef DEBUG
    std::cout << std::hex << "Register values: PC: " <<int(reg.PC) << ", SP: " << int(reg.SP) << ", A: " << int(reg.A) << ", F: " << BIN(reg.F) << ", AF: " << int(reg.AF.getData()) << ", B: " << int(reg.B) << ", C: " << int(reg.C) << ", BC: " << int(reg.BC.getData()) << ", D: " << int(reg.D) << ", E: " << ", DE: " << int(reg.DE.getData()) << ", H:" << int(reg.HL.high) << ", L:" << int(reg.L) << ", HL: " << int(reg.HL.getData()) << std::endl;
#endif*/ 
  while(running) {
...
int eval_opcode(uint16_t opcode,int cycles) {
    ...
    switch(opcode) {
...
#ifdef DEBUG
            std::cout << "LD B " << read_word(reg.PC) << "; " << std::hex << reg.pc-1 << ":" << "Cycles: " << std::dec << cycles << " LD_8_B_pc" << std::endl; 
#endif                
            reg.pc++;
            break;
        case 0x0E:
...
#ifdef DEBUG
            std::cout << "LD C " << read_word(reg.PC) << "; " << std::hex << reg.pc-1 << ":" << "Cycles: " << std::dec << cycles << " LD_8_C_pc" << std::endl; 
#endif                
            reg.pc++;
            break;
        case 0x16:
...
#ifdef DEBUG
            std::cout << "LD D " << read_word(reg.PC) << "; " << std::hex << reg.pc-1 << ":" << "Cycles: " << std::dec << cycles << " LD_8_D_pc" << std::endl; 
#endif                
            reg.pc++;
            break;
        case 0x1E:
...
#ifdef DEBUG
            std::cout << "LD E " << read_word(reg.PC) << "; " << std::hex << reg.pc-1 << ":" << "Cycles: " << std::dec << cycles << " LD_8_E_pc" << std::endl; 
#endif                
            reg.pc++;
            break;
        case 0x26:
            LD_8_n_nn(cycles,reg.H,reg.PC);
#ifdef DEBUG
            std::cout << "LD H " << read_word(reg.PC) << "; " << std::hex << reg.pc-1 << ":" << "Cycles: " << std::dec << cycles << " LD_8_H_pc" << std::endl; 
#endif                
            reg.pc++;
            break;
        case 0x2E:
...
#ifdef DEBUG
            std::cout << "LD L " << read_word(reg.PC) << "; " << std::hex << reg.pc-1 << ":" << "Cycles: " << std::dec << cycles << " LD_8_L_pc" << std::endl; 
#endif                
            reg.pc++;
            break;
        case 0x7F:
        ...
        case 0xCB:
            ...
            uint16_t opcode2 = read_byte(reg.pc++);
