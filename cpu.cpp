...
int main() {
    ...
    //int d = 0x08;    
    int cycles = 0;
    uint16_t opcode = read_word(reg.pc++);
    uint8_t temp;
    int running  = 1;
    while(running) {
    switch(opcode) {
    ...
    //Shifts
         ...
         case 0xCB3E:
            ...
            break;  
         case 0xCB47:
            cmpbit_b_r(cycles,reg.F,reg.A,reg.pc++);
            break;
         case 0xCB40:
            cmpbit_b_r(cycles,reg.F,reg.B,reg.pc++);
            break;
        case 0xCB41:
            cmpbit_b_r(cycles,reg.F,reg.C,reg.pc++);
            break;
          case 0xCB42:
            cmpbit_b_r(cycles,reg.F,reg.D,reg.pc++);
            break;
        case 0xCB43:
            cmpbit_b_r(cycles,reg.F,reg.E,reg.pc++);
            break;  
         case 0xCB44:
            cmpbit_b_r(cycles,reg.F,reg.H,reg.pc++);
            break;
        case 0xCB45:
            cmpbit_b_r(cycles,reg.F,reg.L,reg.pc++);
            break;
        case 0xCB46:
            temp = read_byte(reg.pc++);
            cmpbit_b_r(cycles,reg.F,temp,reg.pc++);
            cycles = 16;
            break;
          case 0xCBC7:
